...
public class TaxThreshold {
	...
	public TaxThreshold(BigDecimal minimumIncome, BigDecimal maximumIncome, BigDecimal fixedTax, BigDecimal variableTax) {
	...
	public BigDecimal calculateTax(BigDecimal grossIncome) {
		...
		if (minimumIncome.compareTo(grossIncome) < 0
			...
			tax = ((grossIncome.subtract(minimumIncome)).multiply(variableTax)).add(fixedTax);
		}
		return tax;
	}
